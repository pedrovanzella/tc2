...
O algoritmo \ref{alg:sim} mostra um pseudo-código de como funciona o laço de
...
\textit{notifier} para receberem notificações de eventos. Nas linhas 8 a 10, é
criado um evento para marcar o fim da simulação e o mesmo é adicionado à
\textit{fila de eventos}. Na linha 12, são inicializados os eventos de chegada
de clientes em cada um dos andares (seção \ref{model:state:building}). Por fim,
o simulador entra em um laço indefinido nas linhas 14 a 17. A cada iteração do
laço é retirado o próximo evento da fila e os \textit{observers} são
notificados. O laço permanece em execução enquanto houverem eventos disponíveis
na fila ou até atingir o tempo limite da simulação.
...
\subsection{StopManager}

  Classe auxiliar que implementa um gerenciador de paradas. Provê os seguintes métodos:
...
\subsection{Building} \label{model:state:building}
