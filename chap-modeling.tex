...
\textit{setters}.} e detalhes de implementação do simulador de elevadores.
...
No exemplo do algoritmo~\ref{alg:config} \unsure{Isto não é um algoritmo. Devemos listar dentro de um algorithm? Como mudar o caption?}são definidos alguns cenários. No
...
\begin{center}
\begin{algorithmic}[1]
\Function{run}{$scenario, clock, statistics, dispatcher, eventQueue$}
  \State $building \leftarrow $\Call{Building}{$scenario$}
  \State
  \State $dispatcher.$\Call{Subscribe}{$building$}
  \State $dispatcher.$\Call{Subscribe}{$clock$}
  \State $dispatcher.$\Call{Subscribe}{$statistics$}
  \State
  ...
  \State
  \While{$statistics.$\Call{KeepRunning}{}() and $eventQueue.\Call{HasNextEvent}{}()$}
    \State $nextEvent \gets eventQueue.$\Call{Pop}{}
    \State $dispatcher.$\Call{Broadcast}{$nextEvent$}
  \EndWhile
\EndFunction
\end{algorithmic}
\end{center}
...
\begin{description}
  \item[Inicializar da lista de chegadas de clientes] \hfill \\
...
void Building::notify(const std::shared_ptr<const Event> event) {
  ...
  \item[Reagir à chegada de um novo cliente] \hfill \\
    ...
    trata o evento de \textit{chegada de cliente}. Nas linhas 2 e 3 são
    identificados o cliente e o andar em que ocorreu o evento de sua chegada. Na
    linha 4 o cliente é adicionado à fila do andar correspondente a onde está -
    retornando a direção em que vai se locomover dentro do prédio. Caso não
    exista uma parada programada de um elevador no andar e na direção (linha 6),
    será necessário escolher um elevador para atender este cliente. Neste
    momento, o \textit{algoritmo de agendamento} é invocado (linha 7) e a
    decisão é informada ao \textit{gerenciador de paradas} (linha 9). Por fim,
    nas linhas 12 a 14 é gerado um novo evento futuro de \textit{chegada de
    cliente} para o andar onde o evento ocorreu, de modo a repopular
    constantemente as filas de cada andar.
...
Ambos são implementados em classes próprias, que herdam de uma classe
...
\unsure{Onde falamos de por que é importante excluir elevadores do schedule às
  vezes? (Vanzella)}
Excluir um elevador é importante, como foi falado na Seção XXX. Há um método
...
\unsure{Pedro, esta seção é tua. (Taschetto)}
\lipsum[5]