...
int SimpleScheduler::schedule(
  ...
  auto the_chosen_one = elevators.first();
  ...
  \end{minted}
  \caption{\textit{Simple Scheduler}.}
  \label{alg:scheduler:simple}
\end{algorithm}

\subsection{\label{model:schedulers:planning}Planning}
A idéia por trás do algoritmo de Planning é simples, ainda que sua implementação
possa parecer complexa, como visto nos
Algoritmos~\ref{alg:scheduler:planning:chamada}~
e~\ref{alg:scheduler:planning:steps}.

Primeiramente faz-se uma cópia profunda do simulador inteiro, contendo todo o
estado atual da simulação, bem como uma cópia da lista de clientes, e de
elevadores, excluindo-se um possível elevador que esteja cheio. Isto é feito
pelo Algoritmo~\ref{alg:scheduler:planning:chamada}. Ele então chama o
Algoritmo~\ref{alg:scheduler:planning:steps}.

Este segundo algoritmo vai agendar um elevador para atender um cliente na cópia
da simulação que recebeu, e avançar o relógio da mesma até o ponto em que o
elevador teria atendido este chamado. Isto é feito iterativamente, já que o
elevador pode ter uma agenda complexa, com várias paradas em diferentes andares.
O custo calculado para este elevador é proporcional ao tempo de simulação que
ele demora entre seu estado atual e o ponto em que atenderia o cliente.

O custo total é calculado chamando-se recursivamente esta função, com uma nova
cópia desta cópia do simulador, agora em seu estado modificado, para os demais
elevadores e clientes.

Deste modo, gera-se algo como uma árvore de execuções, onde cada nodo é uma
combinação elevador-cliente, associado de um custo proporcional ao tempo de simulação.

Cada nodo soma a si o custo do seu filho de menor custo. Desta forma, no retorno
da recursão, tem-se sempre o menor custo para um nodo. Ao fim da
recursão~-~\textit{i.e.} quando o horizonte de \textit{planning} for
encontrado~-~, tem-se automaticamente o par elevador-custo de menor custo. Este
é o próximo passo que deve ser tomado: agendar este elevador para o primeiro
cliente da fila.

\begin{algorithm}[htb!]
  \centering
  \begin{minted}[linenos,fontsize=\small]{c++}
    int PlanningScheduler::schedule(
        CostFunction costFunction,
        Building building,
        Client client,
        int elevatorToExclude) {
  ...
  return the_answer_to_life_the_universe_and_everything.getNumber();
}
  \end{minted}
  \caption{\textit{Planning Scheduler}: chamada.}
...
\begin{algorithm}[htb!]
  \centering
  \begin{minted}[linenos,fontsize=\small]{c++}
pair<Elevator, float> PlanningScheduler::calculate(
      Simulator simulator,
      Clients clients,
      int elevatorToExclude) {
  ...
  for (auto elevator : elevators) {
    ...
    Direction direction = client.getArrivalFloor() < client.getDestination() ?
  ...
  \end{minted}
  \caption{\textit{Planning Scheduler}: passos seguintes.}
...
Listagem~\ref{alg:graphs:output}. Exemplos destes gráficos podem ser vistos a
seguir, nas
Figuras~\ref{fig:graphs:arrivalsperfloor},~\ref{fig:graphs:averagewaittime},
~\ref{fig:graphs:clientsperelevator},~\ref{fig:graphs:dropoffsPerFloor}~
e~\ref{fig:graphs:averageTravelTime}.

\begin{figure}[htb!]
  \centering
  \includegraphics[scale=0.8]{img/results/Low-rise/5_Planning_Random/arrivalsPerFloor.eps}
...
\begin{figure}[htb!]
  \centering
  \includegraphics[scale=0.8]{img/results/Low-rise/5_Planning_Random/averageWaitTime.eps}
...
\begin{figure}[htb!]
  \centering
  \includegraphics[scale=0.8]{img/results/Low-rise/5_Planning_Random/clientsPerElevator.eps}
...
\begin{figure}[htb!]
  \centering
  \includegraphics[scale=0.8]{img/results/Low-rise/5_Planning_Random/dropoffsPerFloor.eps}
